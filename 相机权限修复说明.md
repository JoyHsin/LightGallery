...
在 `LightGallery.xcodeproj/project.pbxproj` 中添加了相机使用说明：
...
修改了 `LightGallery/Views/IDPhotoEditorView.swift` 中的 `CameraPicker`：
...
- `LightGallery.xcodeproj/project.pbxproj` - 项目配置文件（添加了相机权限描述）
- `LightGallery/Views/IDPhotoEditorView.swift` - 证件照编辑器视图（改进了相机权限处理）
